...
def predict_from_folder(dataset, fold:int , indir , outdir , rule = 'both'):
    ...
    for checkpoint in checkpoint_list:  
        ...
        pred_list = [x.replace('.nii.gz',"") for x in os.listdir(indir) if x.endswith('.nii.gz')]
        output_folder_list = [join(checkpoint_output_folder, x.replace("_0000","")) for x in pred_list]
